...
export class GxUtils
{
    ...
    static GetCentroid(Planes:Plane[])
    {
    ...
    static Rotate(pt: Point, around: Point, angle: Angle)
    {
    ...
    static Rotate2DPoint(x0: number, y0: number, x1: number, y1: number, theta: number)
    {
    ...
    static TransformPlanes(planes: Plane[], transformation: Transformation): Plane[]
    {
    ...
    static ApplyRepeatTransform(Planes: Plane[], transformation: Transformation, aroundPoint: Point = new Point(0, 0, 0)): Plane[]
    {
    ...
    static ComputeDistance(pointA: Point, pointB: Point): number
    {

        return Math.sqrt(Math.pow(pointA.x -pointB.x,2) + Math.pow(pointA.y -pointB.y, 2) + Math.pow(pointA.z -pointB.z,2));
    }

    static Translate(planes: Plane[], translation: Point, centroid: Point = new Point(0, 0, 0)): Plane[]
    {
        if (centroid == null)
        {
    ...
    static Zoom(planes: Plane[], zoom: Scale)
    {
        let centroid = GxUtils.GetCentroid(planes);
    ...
    static RotatePlanes(Planes: Plane[], angle: Angle): Plane[] 
    {
        ...
        for(let plCnt=0; plCnt < planes.length; plCnt++)
        {
            ...
            {
                let pt = planes[plCnt].Points[ptCnt];
                let newPt: Point = new Point(0, 0, 0);
                if (angle != null)
                {
    ...
    static GetInclinationWithXZPlane(plane: Plane): number
    {
        ...
        let cosAlpha = Math.abs(
            (a1 * a2) + (b1 * b2) + (c1 * c2)) / 
            (Math.sqrt((a1 * a1) + (b1 * b1) + (c1 * c1) * Math.sqrt((a2 * a2) + (b2 * b2) + (c2 * c2) )));
    ...
    static ApplyRepeatHints(Planes: Plane[], repeatHints: RepeatHint[])
    {
        ...
        for (let xRepeater=0; xRepeater < xRepeatHint.RepeatTimes; xRepeater++)
        {
            for (let yRepeater=0; yRepeater < yRepeatHint.RepeatTimes; yRepeater++)
            {
                for (let zRepeater=0; zRepeater < zRepeatHint.RepeatTimes; zRepeater++)
                {
                    ...
                    new Point(xRepeater * xRepeatHint.SpaceDistance,
                         yRepeater * yRepeatHint.SpaceDistance, 
                         zRepeater * zRepeatHint.SpaceDistance);
